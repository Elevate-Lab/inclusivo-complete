...
const Lists = ({type}) => {
    ...
    return(
        ...
        {isMore && <Grid item xs={12} className={classes.buttonContainer}>
            {!isMoreClicked ? (<Button style={{ color: "red", background: "rgba(255, 234, 236, 1)",width: "100px"}} onClick={handleViewMore}>
                View All
            </Button>
            ) : (<Button style={{ color: "red", background: "rgba(255, 234, 236, 1)",width: "100px"}} onClick={handleViewMore}>
                Close
            </Button>
            )}
        </Grid>
